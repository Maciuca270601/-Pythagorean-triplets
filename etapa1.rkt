...
(define (multiply M V)
  (multiply_helper M V null)
  )

(define (multiply_helper M V Result)
  (cond
    [(null? M) Result]
    [else (multiply_helper (cdr M) V (append Result (list (dot-product (car M) V))))]
    )
  )
...
; indexmin nivel 3 = (index_min 2) + 1
