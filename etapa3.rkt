...
(define (key n)
  (let* ([e (cadr (get-nth-quadruple n))]
         ...
         [mylist (list (a1 e f) (b1 e f) (c1 e f)
                       (a2 e f) (b2 e f) (c2 e f)
                       (a3 e f) (b3 e f) (c3 e f))]
        )
    (map (lambda (x) (modulo x 27)) mylist))
 )
...
(define (message->codes message)
  (map (lambda (x) (if (= x 32) 0 (- x 96))) (map char->integer (string->list message)))
 )
...
(define (codes->message codes)
  (list->string (map integer->char (map (lambda (x) (if (= x 0) 32 (+ x 96))) codes)))
  )
...
(define (extend-key key size)
  (let myextend ( [buffer key]
                  [counter 1]
                  [result null]
                 )
    (cond
      [(> counter size) (reverse result)]
      [(null? buffer) (myextend (cdr key) (+ counter 1) (cons (car key) result))]
      [else (myextend (cdr buffer) (+ counter 1) (cons (car buffer) result))])
    )
 )
...
(define obtain-opposite-code
  (lambda (f)
    (lambda (code key)
      (let* ( [size (max (length code) (length key))]
              [code (extend-key code size)]
              [key (extend-key key size)]
              )
        (map (lambda (x y) (modulo (f x y) 27)) code key))
     )
   )
 )
...
(define obtain-opposite-message
  (lambda (f)
    (lambda (code key) (codes->message ((obtain-opposite-code f) (message->codes code) key)))))


(define encrypt-message
  (obtain-opposite-message +))
